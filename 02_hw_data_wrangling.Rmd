...
knitr::kable(party_incumbent_promote, col.names = c("Party    |", "| Incumbency Status   |", "| Self-Promotion Ad Proportion"))
...
The candidates that sponsor the most promoting ads on FB are Democratic incumbent candidates (85.4%).
...
Most of the candidates had less than 10 million impressions on FB.
...
fb_top_plot <- ggplot(fb_top_plot, aes(x = impressions_millions,
                                       y = fct_reorder(cand_name_last, impressions_millions),
                                       fill = party)) +
  geom_col() +
  scale_fill_manual(values = c(R = "red", D = "blue")) +
  labs(title = "Top House Candidates by FB Ad Impressions",
       x = "FB Ad Impressions (millions)",
       y = "Candidate")
